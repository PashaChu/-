<template>
    ...
    <div v-if="selectedHall" class="modal">
      <div class="modal-content">
          ...
          <div class="modal-inner">
            <div class="modal-input">
              ...
              <input type="text" id="editedId" v-model="editedhall.id" disabled />
            </div>
            <div class="modal-input">
              <label for="editedLogin">Name:</label>
              <input type="text" id="editedLogin" v-model="editedHall.name" />
            </div>
          </div>
          <button class="button-40" @click="updateHall">Обновить</button>
        </div>
      </div>
  </template>
...
export default {
 ...
 methods: {
      async getHalls() {
        try {
          const response = await fetch('http://localhost:8080/halls/table');
        ...
        async deleteHall(hallId) {
          ...
          try {
            await fetch(`http://localhost:8080/halls/table/${hallId}`, {
              method: 'DELETE',
              headers: {
        ...
        updateHallData(field, event) {
          this.hallData[field] = event.target.innerText;
        },
        addHall() {
          fetch("http://localhost:8080/halls/table", {
            method: "POST",
            headers: {
              "Content-Type": "application/json"
            },
            body: JSON.stringify({
              name: this.hallData.name
            })
          })
        ...
        async updateHall() {
          try {
            const response = await fetch('http://localhost:8080/halls/table', {
              ...
              body: JSON.stringify(this.editedHall),
            });
